...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1500 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 1000);
  }
  ...
  int stat_bonus(Depth d) {
    return std::min((8 * d + 275) * d - 275 , 2044);
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((20.84 + std::log(Threads.size()) / 2) * std::log(i));
}
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              int tr = sigmoid(prev, 2, 8, 88, 128, 1);
              trend = (us == WHITE ?  make_score(tr, tr / 2)
                                   : -make_score(tr, tr / 2));

              int opt = sigmoid(prev, 6, 16, 140, 14112, 185);
          ...
          && !mainThread->stopOnPonderhit)
      {
          double fallingEval = (71 + 12 * (mainThread->bestPreviousAverageScore - bestValue)
                                    +  6 * (mainThread->iterValue[iterIdx] - bestValue)) / 780.5;
          ...
          double complexPosition = std::clamp(1.0 + (complexity - 275) / 1817, 0.5, 1.5);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        && depth <= 7
        && eval < alpha - 360 - 255 * depth * depth)
    {
    ...
    if (   !PvNode
        ...
        && (ss-1)->statScore < 14880
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 15 * depth - improvement / 16 + 205 + complexity / 27
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 149, 5) + depth / 3 + 4 - (complexity > 642);
    ...
    probCutBeta = beta + 189 - 45 * improving;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !pos.empty(to_sq(move))
                  ...
                  && lmrDepth < 6
                  && !ss->inCheck
                  && ss->staticEval + 302 + 172 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 11
                  && ss->staticEval + 140 + 132 * lmrDepth + history / 60 <= alpha)
                  continue;

              // Prune moves with negative SEE (~3 Elo)
              if (!pos.see_ge(move, Value(-24 * lmrDepth * lmrDepth - 20 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~0 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5540)
              extension = 1;
      }
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          doDeeperSearch = value > (alpha + 74 + 12 * (newDepth - d));
          didLMR = true;
      }
      else
      {
